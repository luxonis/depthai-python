...
template <typename Type, size_t Extent> struct type_caster<dai::span<Type, Extent>> {
  using span_type = typename dai::span<Type, Extent>;
...
template <typename Type> struct type_caster<dai::span<Type, dai::dynamic_extent>> {
  using span_type = typename  dai::span<Type, dai::dynamic_extent>;
  using value_conv = make_caster<Type>;
  using value_type = typename std::remove_cv<Type>::type;
...
template <> struct type_caster<dai::span<const uint8_t, dai::dynamic_extent>> {
  using span_type = typename  dai::span<const uint8_t, dai::dynamic_extent>;
...
template <> struct type_caster<dai::span<uint8_t, dai::dynamic_extent>> {
  using span_type = typename  dai::span<const uint8_t, dai::dynamic_extent>;
  PYBIND11_TYPE_CASTER(dai::span<uint8_t>, _("buffer"));
  ...
  bool load(handle src, bool convert) {
    ...
    value = dai::span<uint8_t>((uint8_t*)req.ptr, req.size*req.itemsize);
    return true;
  }

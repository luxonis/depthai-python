...
def annotate_frame(frame: np.ndarray, detections: ImgLandmarks) -> None:
    if detections.landmarks:
        pose_points_pairs = convert_to_absolute_points(frame, detections.landmarks[0])
        draw_detected_pose(frame, pose_points_pairs, detections.landmarks_indices[0], detections.pairs)


def convert_to_absolute_points(frame: np.ndarray,
...
def draw_detected_pose(frame: np.ndarray, pose_points_pairs: np.ndarray, indices: List[int],
                       pairs: List[Tuple[int, int]], thickness: int = 2) -> None:
    keypoints = set()

...
parser.add_argument("-m", "--mesh_dims", type=str, default="2x3", help="mesh dimensions widthxheight (default=%(default)s)")
parser.add_argument("-r", "--resolution", type=str, default="500x500", help="preview resolution (default=%(default)s)")
...
if not match:
    print(f"Mesh dimensions format incorrect '{args.resolution}' !!!")    
    sys.exit()
...
if not match:
    print(f"Resolution format incorrect '{args.resolution}' !!!")    
    sys.exit()
...
def create_pipeline(mesh):
    ...
    manip.setWarpMesh(mesh, mesh_w, mesh_h)
    manip.setMaxOutputFrameSize(maxFrameSize)
    camRgb.preview.link(manip.inputImage)
    # Output warped
    xout_warped = pipeline.create(dai.node.XLinkOut)
    xout_warped.setStreamName('warped')
    manip.out.link(xout_warped.input)                                                                                                                    
    return pipeline

...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        disparity = stereo.compute(edgeLeftFrame, edgeRightFrame)
        cv2.imshow("test", disparity)
        #cv2.imshow(edgeLeftStr, edgeLeftFrame)
        #cv2.imshow(edgeRightStr, edgeRightFrame)
        img = cv2.resize(edgeRgbFrame, (0, 0), fx=0.5, fy=0.5)
        img = cv2.bilateralFilter(img, 15, 75, 75)

        corners = cv2.goodFeaturesToTrack(img, 6, 0.2, 50)
        corners = np.int0(corners)
        for corner in corners:
            x, y = corner.ravel()
            cv2.circle(img, (x, y), 5, (255, 0, 0), -1)

        cv2.imshow(edgeRgbStr, img)

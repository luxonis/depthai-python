    ...
    while key != ord('q'):
        ...
        if key >= 0 or (not testFourPt and abs(rotateRate) > 0.0001):
            ...
            if testFourPt:
                ...
                iter = iter + 1
                round = iter % (2 * w + 2 * h)
                if (round >= 0) and (round < w):
                    centerX = round
                    centerY = 0
                if (round >= w) and (round < (w + h)):
                    centerX = w - 1
                    centerY = round - w
                if (round >= (w + h)) and (round < (2 * w + h)):
                    centerX = 2 * w - (round - h)
                    centerY = h - 1
                if (round >= (2 * w + h)) and (round < (2 * w + 2 * h)):
                    centerX = 0
                    centerY = (2 * w + 2 * h) - round

                rotatedRect = ((centerX, centerY), (w, h), angleDeg)
                rr = dai.RotatedRect()

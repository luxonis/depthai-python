...
import math
import os
from read_write_pfm import read_pfm

datasetDefault = str((Path(__file__).parent / Path("models/dataset")).resolve().absolute())
parser = argparse.ArgumentParser()
parser.add_argument("-p", "--dataset", nargs="?", help="Path to dataset", default=datasetDefault)
parser.add_argument("-d", "--debug", action="store_true", help="Enable debug outputs.")
parser.add_argument("-e", "--evaluate", action="store_true", help="Output GT evaluation.")
...
parser.add_argument("--download", action="store_true", help="Downloads the 2014 Middlebury dataset.", default=False)
args = parser.parse_args()

if not Path(args.dataset).exists():
    if not args.download:
        import sys
        raise FileNotFoundError(f"Dataset path does not exist, use the --dataset argument to specify the path.")
    else:
...
def download_2014_middlebury(data_path):
    ...
    for f in files:
        ...
        z.extractall(os.path.join(data_path, f[:-4]))

if args.download:
    download_2014_middlebury(args.dataset)
    ...
    while True:
        ...
        if in_q_list:
            ...
            for i, q in enumerate(in_q_list):
                path = os.path.join(dataset.get(), f"im{i}.png")

...
xOutBenchmark = pipeline.create(dai.node.XLinkOut)

xinFrame.setStreamName("inFrame")
nnOut.setStreamName("nn")
xOutBenchmark.setStreamName("outBenchmark")
...
benchmarkIn.passthrough.link(nnOut.input)
benchmarkIn.report.link(xOutBenchmark.input)
...
with dai.Device(pipeline) as device:
    ...
    qReport = device.getOutputQueue(name="outBenchmark", maxSize=4, blocking=False)
    ...
    while cap.isOpened() and i < 50:
        ...
        if cv2.waitKey(1) == ord('q'):
            break
        i += 1
    outReport = qReport.get()
    print(f"FPS was {outReport.fps}")
